...
const { botfdp } = require('./src/botfdp')
const { informa } = require('./src/informa')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador!',
                                        daftarB: `[ REGISTRE-SE ]\nOlá !\nVoce  ainda nao está registrado com o banco de dados do bot loser! \n\nComando : ${prefix}rg nome|idade\nExemplo : ${prefix}rg Loser|18`,
				}
			}
			...
			const ownerNumber = ["558499034946@s.whatsapp.net"] // Nomer Owner
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'LOSERbots'//Nama Bot
                        const ownerName = 'Loserzin<3'//Nama Owner
			...
			switch(command) {
			                ...
			                const uangku = checkATMuser(sender)
                                ...
                                case '18+menu':
                                        ...
                                        break
                               case 'informa':
		                                client.sendMessage(from, informa(prefix, sender), text, {quoted: mek})
                                        break
                               case 'botfdp':
		                                client.sendMessage(from, botfdp(prefix, sender), text, {quoted: mek})
                                        break
                                case 'downloadermenu':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				case 'rebaixar':
					...
					break
                                case 'randomhentai':
                                        ...
                                        break
                              case 'wallpaper':
					            data = fs.readFileSync('.src/wallpaper.js');
		...
		case 'glass':                                        
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
              ...
              client.updatePresence(from, Presence.composing) 
                random = `${Math.floor(Math.random() * 100)}`
               hasil = `O quanto você é gostoso(A)?\n\nVocê é: *${random}%* Gostoso(A)`
              reply(hasil)
                break
	    case 'kapankah':
               client.updatePresence(from, Presence.composing) 
